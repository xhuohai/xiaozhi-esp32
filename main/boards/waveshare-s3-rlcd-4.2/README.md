...
长时间运行后固件可能卡住。NTP 对时成功后，本地时间到达 `DAILY_REBOOT_HOUR`:`DAILY_REBOOT_MINUTE`（默认 `03:00`）且设备空闲时，走现有 `Application::Reboot()` 重启一次。

- 正在对话、番茄钟或播歌时会等到空闲再重启
- 开机两分钟内不触发，避免刚在 3 点重启又撞上同一分钟
- `secret_config.h` 里把 `DAILY_REBOOT_HOUR` 设为 `-1` 可关闭
